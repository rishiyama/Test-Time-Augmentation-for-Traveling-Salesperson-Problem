...
class TSP_net(nn.Module): 
    ...
    def __init__(self, embedding, nb_nodes, input_pe, dim_input_nodes, dim_emb, dim_ff, nb_layers_encoder, nb_layers_decoder, nb_heads, max_len_PE,
                 batchnorm=True):
        ...
        # check type
        assert self.embedding in ['dim_input_nodes', 'nb_nodes']
        assert self.input_pe in [True, False]
        
        # input embedding layer
        if self.embedding == 'dim_input_nodes':
            self.input_emb = nn.Linear(dim_input_nodes, dim_emb)
        elif self.embedding == 'nb_nodes':
            self.input_emb = nn.Linear(nb_nodes, dim_emb)
    ...
    def forward(self, x, B, greedy, beamsearch):
        ...
        if self.embedding == 'nb_nodes':
            x = torch.cdist(x, x, p=2) # size(x)=(bsz, nb_nodes, nb_nodes)
        ...
        if self.input_pe:
            for t in range(h.shape[1]):
                h[:, t] = h[:, t] + self.PE[t].expand(bsz, self.dim_emb)

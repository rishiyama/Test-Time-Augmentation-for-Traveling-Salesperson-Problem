...
def get_args():
    ...
    parser.add_argument('--beam_width', required=True, choices=[1, 100, 1000, 2500], type=int, help='beamsize width')
    ...
    parser.add_argument('--batch_size_perm', default=100, type=int, help='batch size for permutation')
    parser.add_argument('--perm_method', default="swap_k", type=str, help='method for permutation')
    parser.add_argument('--num_k', default=None, type=int, help='number of k for permutation')
...
def generate_permutations(n, batch=1, _method="random", k=2):
    ...
    if _method == "random":
        perms = torch.Tensor([]).long()
        for i in range(batch):
            perms = torch.cat((perms, torch.randperm(n).unsqueeze(0)), 0)
            
    elif _method == "swap_2":
        
        
        base = torch.arange(n)
        perms = base.repeat(batch,1)

        for i in range(batch):
            a, b = random.sample(range(n), 2)
            perms[i][a], perms[i][b] = base[b], base[a]
    ...
    #             perms[i][indices[j]] = base[indices[j]]

            
    else:
        raise Exception('Unknown permutation method')
            
    return perms 

def batch_permutation(x, batch_size=1, hold_first=True, perm_method="swap_2", k=2):
    """
    Generate a batch of permutations of the nodes of x
    Inputs : x of size (bsz, nb_nodes, 2) batch of tsp tour instances
            batch_size : number of permutations to generate
    Output : x of size (bsz*batch_size, nb_nodes, 2) batch of tsp tour instances
    """
    bsz = x.shape[0]
    assert bsz == 1
    ...
    perms = generate_permutations(nb_nodes, batch_size, _method=perm_method, k=k)
    
    if hold_first:
        first_x = x[0].detach().clone()
    ...
    if hold_first:
        x[0] = first_x
        perms[0] = torch.arange(nb_nodes)
        
        
    return x, perms


def run_test(model_baseline, nb_nodes, bsz, _beam_width, _greedy, _beamsearch, permed_batch_size=100, _perm_method="swap_2", num_k=2):
    ...
    results_permed_length = torch.Tensor([]).to(device)
    results_permutations = torch.Tensor([])
    ...
    for step in range(0,args.nb_batch_eval):
        ...
        y, perms = batch_permutation(x, batch_size=permed_batch_size, hold_first=True, perm_method=_perm_method, k=num_k)
        results_permutations = torch.cat((results_permutations, perms), 0)
        
        
        x = y.to(device)
        x_len_concorde = x_len_concorde.repeat_interleave(permed_batch_size,dim=0)
        with torch.no_grad():
            ...
            if greedy:
                ...
                results_permed_length = torch.cat((results_permed_length, L_greedy), 0)
                
                L_greedy_min_index = L_greedy.argmin()
                L_greedy = L_greedy[L_greedy_min_index].unsqueeze(0)
                scores_greedy = scores_greedy[L_greedy_min_index].unsqueeze(0)
                x_len_concorde = x_len_concorde[L_greedy_min_index].unsqueeze(0) 
...
if __name__ == "__main__":
    ...
    PERMED_BACTH_SIZE=input_args.batch_size_perm
    PERM_METHOD=input_args.perm_method
    ...
    print(f"PERMED_BACTH_SIZE: {PERMED_BACTH_SIZE}, PERM_METHOD: {PERM_METHOD}, NUM_K: {NUM_K}")
    ...
    results, result_text = run_test(model_baseline, nb_nodes=args.nb_nodes, bsz=args.bsz, _beam_width=B, _greedy=greedy, _beamsearch=beamsearch, permed_batch_size=PERMED_BACTH_SIZE, _perm_method=PERM_METHOD, num_k=NUM_K )
    length_data = results.cpu().numpy()
    print(length_data)
    
    
    
    # MODEL_NAME = MODEL_NAME + f'_{DATA_TYPE}_PBS_{PERMED_BACTH_SIZE}' +  f'_PM_{PERM_METHOD}_{NUM_K}'
    MODEL_NAME = MODEL_NAME + f'_{DATA_TYPE}'

    CHECKPOINT_DIR = CHECKPOINT_DIR +  f'_PBS_{PERMED_BACTH_SIZE}' +  f'_PM_{PERM_METHOD}'
    
    # os.mkdir(f'results/{model_baseline}', exist_ok=True)
    os.makedirs(f'results_tta_{CHECKPOINT_DIR}/{TEST_DATA}/{MODEL_NAME}', exist_ok=True)
    
    if greedy:
        # np.save(f'results/{MODEL_NAME}/greedy.npy', length_data)
        torch.save(results, f'results_tta_{CHECKPOINT_DIR}/{TEST_DATA}/{MODEL_NAME}/greedy.pt')
        
        file = open(f'results_tta_{CHECKPOINT_DIR}/{TEST_DATA}/{MODEL_NAME}/greedy_result.txt',"w",1) 
        file.write(result_text)
        file.close()
        
    if beamsearch:
        # np.save(f'results/{MODEL_NAME}/beamsearch_{B}.npy', length_data)
        torch.save(results, f'results_tta_{CHECKPOINT_DIR}/{TEST_DATA}/{MODEL_NAME}/beamsearch_{B}.pt')
    
        file = open(f'results_tta_{CHECKPOINT_DIR}/{TEST_DATA}/{MODEL_NAME}/beamsearch_{B}_result.txt',"w",1) 
        file.write(result_text)
        file.close()